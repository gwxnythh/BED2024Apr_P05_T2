document.addEventListener('DOMContentLoaded', async function() {
    ...
    const contentTitle = document.getElementById('playlist-title');
    const titleElement = document.createElement('h2');
    titleElement.textContent = playlistData.Title   ;
    contentTitle.appendChild(titleElement);

    const cardContainer = document.getElementById('card-container');
    console.log('list-playlist-contetns.html: ', playlistId)
  
    try {
      const response = await fetch(`http://localhost:3000/playlist/contents/${playlistId}`);
      if (!response.ok) {
        throw new Error('Failed to fetch data');
      }
      ...
      data.forEach(card => {
        ...
        updateLink.href = './contents-update.html';
        ...
        deleteLink.addEventListener('click', async function(event) {
            ...
            try {
              const deleteResponse = await fetch(`http://localhost:3000/contents/${card.VideoId}`, {
                method: 'DELETE'
              });
        ...
        urlParams.set('id', card.VideoId);

        viewPlaylistLink.href = `./video.html?${urlParams.toString()}`;
        viewPlaylistLink.classList.add('inline-btn');
        viewPlaylistLink.textContent = 'View Content';
